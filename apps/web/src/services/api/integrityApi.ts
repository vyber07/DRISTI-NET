...
export async function generateCourtPdf(caseId: string, analyst_comments: string = "Generated by system"): Promise<ApiResponse<any>> {
  const data = await post(`/cases/${caseId}/report/court-pdf`, { analyst_comments });
  return { data, meta: { requestId: "req", timestamp: new Date().toISOString(), durationMs: 0, securityClassification: "REAL" } };
