...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| CLM-001 | System uses AI/NLP for entity extraction. | AI / NLP | `README.md`, Target Architecture | `apps/api/app/services/ocr_adapter.py`, `apps/api/app/services/nlp_adapter.py`, `apps/api/app/services/extract_document.py` | HIGH | **IMPLEMENTED** | Adapters for PaddleOCR and IndicBERT exist and are connected, but lack formal AI evaluation (CER/WER/F1) metrics on a held-out dataset. |
...
| CLM-004 | System uses Kafka for event-driven asynchronous processing. | ARCHITECTURE | Target Architecture | `apps/api/app/services/kafka_bus.py`, `workers/worker.py`, `docker-compose.yml` | HIGH | **IMPLEMENTED** | Kafka adapter and worker implemented. Compose definitions exist but deployment validation pending. Production HA/security (TLS, SASL, ACLs, DLQ) are not implemented. |
